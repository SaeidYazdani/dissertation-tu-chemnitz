...
set "MAIN_SOURCE=main.tex"
set "MAIN_OUTPUT=main.pdf"
if /I "%ACTION%"=="pdfa" (
  set "MAIN_SOURCE=main-pdfa.tex"
  set "MAIN_OUTPUT=main-pdfa.pdf"
)
call :build_document %MAIN_SOURCE% out
if errorlevel 1 (
...
echo.
echo Build complete: %CD%\out\%MAIN_OUTPUT%
exit /b 0
...
if /I "%~1"=="pdfa" set "ACTION=pdfa" & shift & goto :parse_args
if /I "%~1"=="--pdfa" set "ACTION=pdfa" & shift & goto :parse_args
if /I "%~1"=="/pdfa" set "ACTION=pdfa" & shift & goto :parse_args
...
echo   --pdfa, /pdfa              Build PDF/A-2b output as out\main-pdfa.pdf.
